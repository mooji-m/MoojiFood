...
class MainScreenPresenter(
    ...
    override fun onSearchingFood(filter: String) {

        if (filter.isNotEmpty()) {

            val dataShow = foodDao.searchFood(filter)
            mainView!!.refreshFood(dataShow)
        } else {

            val dataShow = foodDao.getAllFood()
            mainView!!.refreshFood(dataShow)
        }
